...
var ColorWheel = /** @class */ (function (_super) {
    ...
    function ColorWheel(element, options) {
        ...
        _this.themeChanged = new wjcCore.Event();
        ...
        _this._tooltip = new wjcCore.Tooltip({
            showAtMouse: true,
            gap: 12
        });
        // handle clicks to select primary/accent colors
        _this._svg.addEventListener('click', function (e) {
            var g = wjcCore.closest(e.target, 'g'), index = g ? g.getAttribute(ColorWheel._PALINDEX) : null;
            if (index) {
