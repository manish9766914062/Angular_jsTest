...
var InfoSource = /** @class */ (function () {
    function InfoSource(shortUrl, keys, listKeys, trimName) {
        this.URL_FORMAT = 'http://services.arcgisonline.com:80/ArcGIS/rest/services/Demographics/SHORT_URL/MapServer';
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            var val = new infoValue(key, null);
            this.values[key] = val;
            // add to list
                ...
                if (data.layers) {
                    ...
                    for (var j = 0; j < drawInfos.length; j++) {
                        ...
                        if (!self.legendExists(drawInfo.label)) {
                            var index = drawInfo.label.indexOf('('), label = drawInfo.label;
                            if (index > 0) {
 ...
 */
    ...
    InfoSource.getTapestryDescription = function (index) {
        var desc = [
            '',
    ...
    InfoSource.convertColor = function (array) {
...
//# sourceMappingURL=InfoSourceModel.js.map