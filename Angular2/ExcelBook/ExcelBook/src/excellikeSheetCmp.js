...
var ExcellikeSheetCmp = /** @class */ (function () {
        ...
        if (flexSheet) {
            ...
            flexSheet.updatedLayout.addHandler(function () {
                if (flexSheet.selection && flexSheet.selection.isValid) {
                    self._getSelectedTable(flexSheet.selection);
                }
                else {
    ...
    ExcellikeSheetCmp.prototype.lbChartTypesInit = function (lbChartTypes) {
        var self = this;
        if (lbChartTypes) {
            lbChartTypes.hostElement.addEventListener('mouseup', function () {
                setTimeout(function () {
                    self._addChart();
                }, 10);
            });
            lbChartTypes.lostFocus.addHandler(function () {
                lbChartTypes.hostElement.style.display = 'none';
            });
        }
    };
    ExcellikeSheetCmp.prototype.cboTableStylesInit = function (cboTableStyles) {
        var self = this;
        if (cboTableStyles) {
            cboTableStyles.selectedIndexChanged.addHandler(function () {
                // apply the table style for the selected table
                if (!self._updatingSelection && self.selectedTable) {
                    var tableStyle = self.flexSheet.getBuiltInTableStyle(cboTableStyles.selectedValue);
                    self.selectedTable.style = tableStyle;
                }
            });
            if (self.selectedTable) {
                var tableStyle = self.flexSheet.getBuiltInTableStyle(self.selectedTable.style.name);
                if (tableStyle) {
    ...
    ExcellikeSheetCmp.prototype._getSelectedTable = function (selection) {
        var rowIndex, colIndx, selectedTable, tableStyle;
        for (rowIndex = selection.topRow; rowIndex <= selection.bottomRow; rowIndex++) {
            for (colIndx = selection.leftCol; colIndx <= selection.rightCol; colIndx++) {
                selectedTable = this.flexSheet.selectedSheet.findTable(rowIndex, colIndx);
                if (selectedTable) {
                    this.selectedTable = selectedTable;
                    tableStyle = this.flexSheet.getBuiltInTableStyle(selectedTable.style.name);
                    if (tableStyle) {
    ...
    /*
    private _generateTableSheet(flexSheet) {
        var data = this.dataSvc.getTableData(10),
            tableStyle: wjcGridSheet.TableStyle,
        ...
        flexSheet.setCellData(3, 1, 'If you want to add an empty table, you can invoke the "addTable" method of FlexSheet.');
        flexSheet.setCellData(4, 1, 'The code snippet is as below:');
        flexSheet.setCellData(5, 1, 'flexSheet.addTable(1, 1, 10, 4, "Table1").');
        flexSheet.setCellData(7, 1, 'FlexSheet also allows to add a table with data source via "addTableFromDataSource" method.');
        flexSheet.setCellData(8, 1, 'The code snippet is as below:');
        flexSheet.setCellData(9, 1, 'flexSheet.addTableFromDataSource(1, 1, dataSource, "Table1").');
        flexSheet.setCellData(11, 1, 'Following sample shows how add a table with data source:');

        flexSheet.applyCellsStyle({
            fontWeight: 'bold',
            color: 'red'
        }, [new wjcGrid.CellRange(5, 1, 5, 3),
                new wjcGrid.CellRange(9, 1, 9, 4)]);

        flexSheet.applyCellsStyle({
            fontStyle: 'italic'
        }, [new wjcGrid.CellRange(4, 1, 4, 2),
                new wjcGrid.CellRange(8, 1, 8, 2),
                new wjcGrid.CellRange(11, 1, 11, 4)]);

        flexSheet.mergeRange(new wjcGrid.CellRange(1, 1, 1, 6));
        flexSheet.mergeRange(new wjcGrid.CellRange(2, 1, 2, 5));
        flexSheet.mergeRange(new wjcGrid.CellRange(3, 1, 3, 5));
        flexSheet.mergeRange(new wjcGrid.CellRange(4, 1, 4, 2));
        flexSheet.mergeRange(new wjcGrid.CellRange(5, 1, 5, 3));
        flexSheet.mergeRange(new wjcGrid.CellRange(7, 1, 7, 6));
        flexSheet.mergeRange(new wjcGrid.CellRange(8, 1, 8, 2));
        flexSheet.mergeRange(new wjcGrid.CellRange(9, 1, 9, 4));
        flexSheet.mergeRange(new wjcGrid.CellRange(11, 1, 11, 4));

        tableStyle = flexSheet.getBuiltInTableStyle('TableStyleDark9');
        table = flexSheet.addTableFromDataSource(12, 1, data, 'Table1', tableStyle, { showTotalRow: true});
    }
    */
    ...
    ], ExcellikeSheetCmp.prototype, "cboTableStyles", void 0);
