...
var DataSvc = /** @class */ (function () {
    ...
    DataSvc_1 = DataSvc;
    Object.defineProperty(DataSvc.prototype, "fields", {
        get: function () {
            return this._fields;
        },
        set: function (value) {
    ...
    DataSvc.prototype.generateSlipData = function (count) {
        var slipData = {}, items = [];
    ...
    DataSvc.prototype.generateOrdersSlipData = function () {
        return [
            {
                productId: 1,
                productName: 'Chai',
    ...
    DataSvc.prototype.getOrderDetail = function (count) {
        var orderDetails = [], parts = ['Widget', 'Gadget', 'Doohickey'], companies = ['Alfreds Futterkiste', 'Ernst Handel', 'Eastern Connection', 'Du monde entier', 'Consolidated Holdings', 'Chop-suey Chinese'], persons = ['Laurence Lebihan', 'Elizabeth Lincoln', 'Victoria Ashworth', 'Patricio Simpson'];
        for (var i = 0; i < count; i++) {
            var orderDate = new Date(2016, i % 12, i % 28);
            orderDetails[i] = {
                orderId: 'PT-0000' + (i + 1),
                partId: DataSvc_1.randBetween(10000, 99999).toString(),
                handlingId: DataSvc_1.randBetween(100, 999).toString(),
                processingStatus: 'Testing',
                purchaseId: DataSvc_1.randBetween(1000, 9999).toString(),
                completed: '',
                orderDate: orderDate,
                ...
                quantity1: DataSvc_1.randBetween(100, 999),
                quantity2: DataSvc_1.randBetween(100, 999),
                quantity3: DataSvc_1.randBetween(100, 999),
                unit: 'copy',
    ...
    DataSvc.prototype.getPurchaseSlip = function () {
        return [
            {
                productId: 'DC0001',
                productName: 'Chai',
    ...
    DataSvc.prototype.getOrders = function (count) {
        var orders = [];
        for (var i = 0; i < count; i++) {
            var product = this.products[Math.floor(Math.random() * 6)], quantity = DataSvc_1.randBetween(100, 999), orderDate = new Date(2016, i % 12, i % 28);
            orders[i] = {
                orderId: 'C' + wjcCore.Globalize.format(i + 1, 'd4'),
    ...
    DataSvc.prototype.getSalesSlip = function (count) {
        var salseSlip = [];
        for (var i = 0; i < count; i++) {
            var product = this.products[Math.floor(Math.random() * 6)], quantity = DataSvc_1.randBetween(10, 50), profitUnitPrice = product.profitUnitPrice * 10, unitPrice = product.unitPrice * 10, salesAmount = unitPrice * quantity, totalProfit = profitUnitPrice * quantity;
            salseSlip[i] = {
                id: i + 1,
                productId: product.id,
                productName: product.name,
    ...
    DataSvc.prototype.buildDataMap = function (items) {
        var map = [];
    ...
    DataSvc.randBetween = function (min, max) {
