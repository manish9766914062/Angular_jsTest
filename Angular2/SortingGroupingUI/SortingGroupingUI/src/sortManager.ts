...
export class SortManager {
    _view: wjcCore.ICollectionView; // the ICollectionView being controlled
    _props: string[]; // the properties available to sort on
    _sorts: wjcCore.CollectionView; // the sortDescriptions being edited
     ...
     */
    setView(view: wjcCore.CollectionView, properties?: string[]) {

        // save view
        this._view = wjcCore.asCollectionView(view);
        ...
        for (var i = 0; i < this._view.sortDescriptions.length; i++) {
            var sd = <wjcCore.SortDescription>this._view.sortDescriptions[i];
            sorts.push({
                property: sd.property,
                ascending: sd.ascending
            });
        }
        this._sorts = new wjcCore.CollectionView(sorts);
    ...
    commit(save: boolean) {
        ...
        for (var i = 0; i < arr.length; i++) {
            var flex = <wjcGrid.FlexGrid>wjcCore.Control.getControl(arr[i]);
            flex.finishEditing();
        }
        ...
        if (save) {
            this._view.deferUpdate(() => {
                ...
                for (var i = 0; i < arr.length; i++) {
                    if (this.properties.indexOf(arr[i].property) > -1 && wjcCore.isBoolean(arr[i].ascending)) {
                        var sd = new wjcCore.SortDescription(arr[i].property, arr[i].ascending);
    ...
    getError(): string {
        if (this._sorts) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (!wjcCore.isBoolean(arr[i].ascending)) return 'Order not specified: "' + prop + '"';
                used.push(prop);
            }
        }
        return null;
     ...
     */
    get sortDescriptions(): wjcCore.ICollectionView {
        return this._sorts;
    }
}