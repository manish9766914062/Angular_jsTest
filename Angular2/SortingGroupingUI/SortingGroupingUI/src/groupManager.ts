...
export class GroupManager {
    ...
    _groups: wjcCore.CollectionView; // the groupDescriptions being edited
    ...
    setView(view: wjcCore.CollectionView, properties?: string[]) {
        ...
        for (var i = 0; i < this._view.groupDescriptions.length; i++) {
            var gd = <wjcCore.PropertyGroupDescription>wjcCore.asType(this._view.groupDescriptions[i], wjcCore.PropertyGroupDescription);
            if (gd) {
                groups.push({
        ...
        this._groups = new wjcCore.CollectionView(groups);
            ...
            this._view.deferUpdate(() => {
                ...
                for (var i = 0; i < arr.length; i++) {
                    if (this.properties.indexOf(arr[i].propertyName) > -1) {
                        var gd = new wjcCore.PropertyGroupDescription(arr[i].propertyName);
     ...
     */
    get groupDescriptions(): wjcCore.ICollectionView {
        return this._groups;
    }
}